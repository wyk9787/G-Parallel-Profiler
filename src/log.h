...
class logger_base {
 public:
  logger_base indent(size_t n, size_t tab_size = 2) { return logger_base(); }

  template <typename T>
  logger_base operator<<(T t) {
    return logger_base();
  }
};
...
#else
#define LOG(color, exit)                                                 \
  (ccutil::logger(exit) << ccutil::SourceColor << "[" << __FILE__ << ":" \
                        << __LINE__ << "] " << color)
